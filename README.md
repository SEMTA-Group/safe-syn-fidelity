...
Step 2: Download the image and store it in the folder [data_syn_DALLE3](data_syn_DALLE3). The PNG image generated by DALLE3 is with 4 channels (RGBA). To allow the object detector to read it, one shall convert it into 3 channel image (RGB). 
* Images in [data_syn_DALLE3](data_syn_DALLE3) are with RGBA
...
Step 3: Trigger [LabelImg](https://pypi.org/project/labelImg/1.4.0/) to perform manual labelling
...
Step 2: Generate the labels into KITTI form

```console
python xml2kitti.py data/Syn_DALLE3_Label/PASCAL_VOC
```

The KITTI label will be stored in the same folder. 
